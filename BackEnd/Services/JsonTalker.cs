...
namespace BackEnd.Services
{
    ...
    {
        public static bool WriteJson(List<Transaction> transactionList)
        {
            try
            {
                string filePath = "../Transaction.json";
                string jsonString = JsonConvert.SerializeObject(transactionList, Formatting.Indented);
        ...
        public static async Task<Double> CallOpenExchangeRatesApi(string currency)
        {
            ...
            await Task.Run(async () =>
            {
                ...
                string parameter = $"&symbols=EUR,{currency}";
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        var exchangeData = JsonConvert.DeserializeObject<Rates>(json);

                        
                        double usdToEur = exchangeData.rates.EUR;
                        double usdToCurrency = 1;
                        PropertyInfo propertyInfo = typeof(RateData).GetProperty(currency);
                        if (propertyInfo != null)
                        {
                            usdToCurrency = (double)propertyInfo.GetValue(exchangeData.rates);
                        }
                        else 
                            Console.WriteLine("taux inconnue");


                        exchangeRate = usdToEur / usdToCurrency;
                    }
                    else
                    {
